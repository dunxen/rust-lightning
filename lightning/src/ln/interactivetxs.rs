...
impl AbortReason {
	pub fn into_tx_abort_msg(self, channel_id: ChannelId) -> msgs::TxAbort {
		let msg = match self {
		    ...
		    AbortReason::DuplicateFundingOutput => "More than one funding (shared) output found",
...
struct NegotiationContext {
	...
	expected_shared_funding_output: Option<(ScriptBuf, u64)>,
...
impl NegotiationContext {
	fn new(
		...
		expected_shared_funding_output: Option<(ScriptBuf, u64)>, tx_locktime: AbsoluteLockTime,
		feerate_sat_per_kw: u32,
	) -> Self {
		NegotiationContext {
			holder_node_id,
	...
	fn set_actual_new_funding_output(
		...
		assert!(self.expected_shared_funding_output.is_some());
		let value = tx_out.value;
		let local_owned = self.expected_shared_funding_output.as_ref().unwrap().1;
		// Sanity check
	...
	fn local_inputs_value(&self) -> u64 {
		self.inputs
			.iter()
			.fold(0u64, |acc, (_, input)| acc.saturating_add(input.prev_output().value))
	}
	...
	fn received_tx_add_output(&mut self, msg: &msgs::TxAddOutput) -> Result<(), AbortReason> {
		...
		let is_shared = if let Some((shared_funding_script, _local_value)) =
			&self.expected_shared_funding_output
		{
			msg.script == *shared_funding_script
		} else {
			false
		};
	...
	fn sent_tx_add_input(&mut self, msg: &msgs::TxAddInput) -> Result<(), AbortReason> {
		...
		let input = InteractiveTxInput::Local(LocalOrRemoteInput {
			serial_id: msg.serial_id,
			input: txin,
			prev_output: tx.output.get(vout).ok_or(AbortReason::PrevTxOutInvalid)?.clone(),
		});
...
impl SharedOwnedOutput {
	...
	fn remote_owned(&self) -> u64 {
		self.tx_out.value.saturating_sub(self.local_owned)
	}
}

/// Represents an output, with information about
/// its control -- exclusive by the adder or shared --, and
/// its ownership -- fully owned by the adder or jointly
#[derive(Clone, Debug, Eq, PartialEq)]
	...
	fn remote_value(&self, local_role: AddingRole) -> u64 {
		match self {
			OutputOwned::Single(tx_out) | OutputOwned::SharedControlFullyOwned(tx_out) => {
				match local_role {
					AddingRole::Local => 0,
					AddingRole::Remote => tx_out.value,
				}
			},
...
impl InteractiveTxInput {
	...
	pub fn local_value(&self) -> u64 {
		match self {
			InteractiveTxInput::Local(input) => input.prev_output.value,
	...
	pub fn remote_value(&self) -> u64 {
		match self {
			...
			InteractiveTxInput::Remote(input) => input.prev_output.value,
		}
	}
}
...
impl InteractiveTxConstructor {
	...
	///
	/// `expected_remote_shared_funding_output`: In the rare case when the local node doesn't
	/// add a shared output, but it expects a shared output to be added by the remote node,
	/// it can specify the script pubkey, used to determine the shared output,
	/// and its (local) contribution from the shared output.
	/// The local value cannot be larger that the actual shared output.
	///
		...
		ES::Target: EntropySource,
	{
		...
		let mut shared_funding_output: Option<(ScriptBuf, u64)> = None;
		for output in &outputs_to_contribute {
			let new_output = match output {
				OutputOwned::Single(_tx_out) => None,
				OutputOwned::SharedControlFullyOwned(tx_out) => {
					Some((tx_out.script_pubkey.clone(), tx_out.value))
				},
				OutputOwned::Shared(output) => {
					// Sanity check
					if output.local_owned > output.tx_out.value {
						return Err(AbortReason::InvalidLowFundingOutputValue);
					}
			...
			if new_output.is_some() {
				if shared_funding_output.is_some() {
					// more than one local-added shared output
					return Err(AbortReason::DuplicateFundingOutput);
				}
				shared_funding_output = new_output;
			}
		}
		if expected_remote_shared_funding_output.is_some() {
			if shared_funding_output.is_some() {
				// one local-added and one remote-expected shared output
				return Err(AbortReason::DuplicateFundingOutput);
			}
			shared_funding_output = Some(expected_remote_shared_funding_output.unwrap());
		}
...
mod tests {
	...
	fn generate_funding_script_pubkey() -> ScriptBuf {
		Builder::new().push_int(33).into_script().to_v0_p2wsh()
	}
	...
	fn generate_non_witness_output(value: u64) -> OutputOwned {
		OutputOwned::Single(TxOut { value, script_pubkey: generate_p2sh_script_pubkey() })
	}
