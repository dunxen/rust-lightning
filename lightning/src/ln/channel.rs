...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider {
	...
	fn commitment_signed_initial<L: Deref>(
		&mut self, signature: Signature, best_block: BestBlock, signer_provider: &SP, logger: &L
	) -> Result<ChannelMonitor<<SP::Target as SignerProvider>::EcdsaSigner>, ChannelError>
	where L::Target: Logger
	{
		if !matches!(self.channel_state, ChannelState::FundingNegotiated) {
			return Err(ChannelError::Close(
				(
		...
		let holder_signer = self.build_holder_transaction_keys();
		let initial_commitment_tx = self.build_commitment_transaction(
			self.holder_commitment_point.transaction_number(), &holder_signer, true, false, logger
		).tx;
		{
			...
			if self.secp_ctx.verify_ecdsa(&sighash, &signature, &self.get_counterparty_pubkeys().funding_pubkey).is_err() {
				return Err(ChannelError::Close(
					(
		...
		let channel_monitor = ChannelMonitor::new(self.secp_ctx.clone(), monitor_signer,
		                                          ...
		                                          &self.channel_transaction_parameters, self.is_outbound(),
		...
		assert!(!self.channel_state.is_monitor_update_in_progress()); // We have no had any monitor(s) yet to fail update!
		if self.holder_commitment_point.advance(&self.holder_signer, &self.secp_ctx, logger).is_err() {
			// We only fail to advance our commitment point/number if we're currently
			// waiting for our signer to unblock and provide a commitment point.
			// If we err here by the time we receive the initial commitment_signed, something has gone wrong.
			debug_assert!(false, "We should be ready to advance our commitment point by the time we receive initial commitment_signed");
...
impl<SP: Deref> OutboundV1Channel<SP> where SP::Target: SignerProvider {
		...
		L::Target: Logger
	{
		...
		let channel_monitor = match self.context.commitment_signed_initial(msg.signature, best_block, signer_provider, logger) {
