...
impl<Signer: WriteableEcdsaChannelSigner> OutboundV1Channel<Signer> {
	...
	) -> Result<(Channel<Signer>, ChannelMonitor<Signer>), (Self, ChannelError)>
	where
		SP::Target: SignerProvider<Signer = Signer>,
		L::Target: Logger
	{
		...
		let validate_holder_commitment_res = self.context.holder_signer.validate_holder_commitment(&holder_commitment_tx, Vec::new());
		if let Err(_) = validate_holder_commitment_res {
		...
		// Promote this InboundV1Channel to a Channel.
...
mod tests {
	...
	fn test_holder_vs_counterparty_dust_limit() {
		...
		let (mut node_a_chan, _) = node_a_chan.funding_signed(&funding_signed_msg, best_block, &&keys_provider, &&logger).map_err(|_| ()).unwrap();
	...
	fn channel_update() {
		...
		let funding_outpoint = OutPoint{ txid: tx.txid(), index: 0 };
		let funding_created_msg = node_a_chan.get_outbound_funding_created(tx.clone(), funding_outpoint, &&logger).map_err(|_| ()).unwrap();
		let (_, funding_signed_msg, _) = node_b_chan.funding_created(&funding_created_msg, best_block, &&keys_provider, &&logger).map_err(|(_, e)| e).unwrap();
