...
use crate::ln::channelmanager::{self, ChannelDetails, ChannelCounterparty, CounterpartyForwardingInfo, PendingHTLCStatus, HTLCSource, SentHTLCId, HTLCFailureMsg, PendingHTLCInfo, RAACommitmentOrder, BREAKDOWN_TIMEOUT, MIN_CLTV_EXPIRY_DELTA, MAX_LOCAL_BREAKDOWN_TIMEOUT};
...
pub(super) trait ChannelInterface<'a, Signer: WriteableEcdsaChannelSigner + 'a> {
	...
	fn temporary_channel_id(&'a self) -> Option<[u8; 32]> {
		self.get_context().temporary_channel_id
	}
	...
	fn should_emit_channel_pending_event(&'a mut self) -> bool {
		self.is_funding_initiated() && !self.get_context().channel_pending_event_emitted
	}

	// Returns whether we already emitted a `ChannelPending` event.
	fn channel_pending_event_emitted(&'a self) -> bool {
		self.get_context().channel_pending_event_emitted
	}

	// Remembers that we already emitted a `ChannelPending` event.
	fn set_channel_pending_event_emitted(&'a mut self) {
		self.get_context_mut().channel_pending_event_emitted = true;
	}
	...
	/// Allowed in any state (including after shutdown)
	fn is_live(&'a self) -> bool {
		self.is_usable() && (self.get_context().channel_state & (ChannelState::PeerDisconnected as u32) == 0)
	}
	...
	fn force_shutdown(&'a mut self, should_broadcast: bool) -> (Option<(OutPoint, ChannelMonitorUpdate)>, Vec<(HTLCSource, PaymentHash, PublicKey, [u8; 32])>) {
		...
		let monitor_update = if let Some(funding_txo) = context.channel_transaction_parameters.funding_outpoint {
			...
			if context.channel_state & (ChannelState::FundingSent as u32 | ChannelState::ChannelReady as u32 | ChannelState::ShutdownComplete as u32) != 0 {
				context.latest_monitor_update_id = CLOSED_CHANNEL_UPDATE_ID;
	...
	/// Allowed in any state (including after shutdown)
	fn get_outbound_forwarding_fee_base_msat(&'a self) -> u32 {
		self.get_context().config.options.forwarding_fee_base_msat
	}

	fn get_channel_details(&'a self, best_block_height: u32, latest_features: InitFeatures) -> ChannelDetails {
		let balance = self.get_available_balances();
		let (to_remote_reserve_satoshis, to_self_reserve_satoshis) =
			self.get_holder_counterparty_selected_channel_reserve_satoshis();
		ChannelDetails {
			...
			counterparty: ChannelCounterparty {
				...
				// Else `ChannelInterface::get_counterparty_htlc_minimum_msat` could return the
				// default `0` value set by `OutboundV1Channel::new`.
				outbound_htlc_minimum_msat: if self.have_received_message() {
					Some(self.get_counterparty_htlc_minimum_msat()) } else { None },
				outbound_htlc_maximum_msat: self.get_counterparty_htlc_maximum_msat(),
			},
			funding_txo: self.get_funding_txo(),
			// Note that accept_channel (or open_channel) is always the first message, so
			// `have_received_message` indicates that type negotiation has completed.
			channel_type: if self.have_received_message() { Some(self.get_channel_type().clone()) } else { None },
			short_channel_id: self.get_short_channel_id(),
			outbound_scid_alias: if self.is_usable() { Some(self.outbound_scid_alias()) } else { None },
			inbound_scid_alias: self.latest_inbound_scid_alias(),
			channel_value_satoshis: self.get_value_satoshis(),
			...
			confirmations_required: self.minimum_depth(),
			confirmations: Some(self.get_funding_tx_confirmations(best_block_height)),
			force_close_spend_delay: self.get_counterparty_selected_contest_delay(),
			is_outbound: self.is_outbound(),
			is_channel_ready: self.is_usable(),
			is_usable: self.is_live(),
			is_public: self.should_announce(),
			inbound_htlc_minimum_msat: Some(self.get_holder_htlc_minimum_msat()),
			inbound_htlc_maximum_msat: self.get_holder_htlc_maximum_msat(),
...
impl<Signer: WriteableEcdsaChannelSigner> OutboundV1Channel<Signer> {
	...
	pub fn get_outbound_funding_created<L: Deref>(mut self, funding_transaction: Transaction, funding_txo: OutPoint, logger: &L) -> Result<(Channel<Signer>, msgs::FundingCreated), (Self, ChannelError)> where L::Target: Logger {
		if !self.is_outbound() {
...
mod tests {
	...
	use crate::ln::channel::{Channel, ChannelInterface, OutboundV1Channel, InboundHTLCOutput, OutboundHTLCOutput, InboundHTLCState, OutboundHTLCState, HTLCCandidate, HTLCInitiator, InboundV1Channel};
