...
use crate::ln::channel::{FundedChannel, OutboundV1Channel, InboundV1Channel, ChannelInterface, ChannelError, ChannelUpdateStatus, UpdateFulfillCommitFetch};
...
pub(super) struct PeerState<Signer: ChannelSigner> {
	/// `channel_id` -> `FundedChannel`.
	///
	/// Holds all funded channels where the peer is the counterparty.
	pub(super) funded_channel_by_id: HashMap<[u8; 32], FundedChannel<Signer>>,
...
impl <Signer: ChannelSigner> PeerState<Signer> {
	...
	fn ok_to_remove(&self, require_disconnected: bool) -> bool {
		...
		self.funded_channel_by_id.is_empty() && self.monitor_update_blocked_actions.is_empty()
	}

	// Returns a count of all channels we have with this peer, including pending channels.
	fn total_channel_count(&self) -> usize {
		self.funded_channel_by_id.len() + self.outbound_v1_channel_by_id.len() +
			self.inbound_v1_channel_by_id.len()
	}

	// Returns a count of all pending channels that have not yet completed handshake and generated
	// funded.
	fn pending_channel_count(&self) -> usize {
		self.inbound_v1_channel_by_id.len() + self.outbound_v1_channel_by_id.len()
	}
}
...
impl ChannelDetails {
	...
	fn from_channel<Signer: WriteableEcdsaChannelSigner>(channel: &FundedChannel<Signer>,
	...
	fn from_inbound_v1_channel<Signer: WriteableEcdsaChannelSigner>(channel: &InboundV1Channel<Signer>,
		best_block_height: u32, latest_features: InitFeatures) -> Self {
	...
	fn from_outbound_v1_channel<Signer: WriteableEcdsaChannelSigner>(channel: &OutboundV1Channel<Signer>,
		best_block_height: u32, latest_features: InitFeatures) -> Self {
	...
	($self: ident, $res: expr, $entry: expr) => {
		match $res {
			...
			Err(e) => {
				...
				let (drop, res) = $self.convert_funded_chan_err(e, $entry.get_mut(), &channel_id);
				if drop {
					$entry.remove_entry();
				}
				break Err(res);
...
macro_rules! try_outbound_v1_chan_entry {
	($self: ident, $res: expr, $entry: expr) => {
		match $res {
			Ok(res) => res,
			Err(e) => {
				let channel_id = $entry.key().clone();
				let (drop, res) = $self.convert_outbound_v1_chan_err(e, $entry.get_mut(), &channel_id);
				if drop {
					$entry.remove_entry();
				}
				return Err(res);
...
macro_rules! remove_inbound_v1_channel {
	($self: expr, $entry: expr) => {
		{
			let channel = $entry.remove_entry().1;
			$self.update_maps_on_pending_chan_removal(&channel.channel_id(), channel.outbound_scid_alias());
			channel
		}
	}
}

macro_rules! remove_outbound_v1_channel {
	($self: expr, $entry: expr) => {
		{
			let channel = $entry.remove_entry().1;
			$self.update_maps_on_pending_chan_removal(&channel.channel_id(), channel.outbound_scid_alias());
	...
	L::Target: Logger,
{
	...
	fn list_funded_channels_with_filter<Fn: FnMut(&(&[u8; 32], &FundedChannel<<SP::Target as SignerProvider>::Signer>)) -> bool + Copy>(&self, f: Fn) -> Vec<ChannelDetails> {
		...
		let mut res = Vec::with_capacity(self.short_to_chan_info.read().unwrap().len());
		{
			...
			for (_cp_id, peer_state_mutex) in per_peer_state.iter() {
				...
				for (_channel_id, channel) in peer_state.funded_channel_by_id.iter().filter(f) {
	...
	pub fn list_channels(&self) -> Vec<ChannelDetails> {
		...
		let mut res = Vec::with_capacity(self.short_to_chan_info.read().unwrap().len());
		{
			...
			for (_cp_id, peer_state_mutex) in per_peer_state.iter() {
				let mut peer_state_lock = peer_state_mutex.lock().unwrap();
				let peer_state = &mut *peer_state_lock;
				for (_channel_id, channel) in peer_state.funded_channel_by_id.iter() {
					let details = ChannelDetails::from_channel(channel, best_block_height,
						peer_state.latest_features.clone());
					res.push(details);
				}
				for (_channel_id, channel) in peer_state.inbound_v1_channel_by_id.iter() {
					let details = ChannelDetails::from_inbound_v1_channel(channel, best_block_height,
						peer_state.latest_features.clone());
					res.push(details);
				}
				for (_channel_id, channel) in peer_state.outbound_v1_channel_by_id.iter() {
					let details = ChannelDetails::from_outbound_v1_channel(channel, best_block_height,
						peer_state.latest_features.clone());
					res.push(details);
				}
			}
	...
	pub fn list_usable_channels(&self) -> Vec<ChannelDetails> {
		...
		self.list_funded_channels_with_filter(|&(_, ref channel)| channel.is_live())
	}
	...
	pub fn list_channels_with_counterparty(&self, counterparty_node_id: &PublicKey) -> Vec<ChannelDetails> {
		...
		if let Some(peer_state_mutex) = per_peer_state.get(counterparty_node_id) {
			...
			return peer_state.funded_channel_by_id
				.iter()
				.map(|(_, channel)|
	...
	fn issue_channel_close_events(&self, channel: &FundedChannel<<SP::Target as SignerProvider>::Signer>, closure_reason: ClosureReason) {
	...
	fn close_channel_internal(&self, channel_id: &[u8; 32], counterparty_node_id: &PublicKey, target_feerate_sats_per_1000_weight: Option<u32>) -> Result<(), APIError> {
		...
		let result: Result<(), _> = loop {
			...
			let mut peer_state_lock = peer_state_mutex.lock().unwrap();
			let peer_state = &mut *peer_state_lock;
			match peer_state.funded_channel_by_id.entry(channel_id.clone()) {
	...
	-> Result<PublicKey, APIError> {
		...
		let (update_res, counterparty_node_id) = {
			let mut peer_state_lock = peer_state_mutex.lock().unwrap();
			let peer_state = &mut *peer_state_lock;
			...
			if let hash_map::Entry::Occupied(chan) = peer_state.funded_channel_by_id.entry(channel_id.clone()) {
				self.issue_channel_close_events(chan.get(), closure_reason);
				let mut chan = remove_channel!(self, chan);
				self.finish_force_close_channel(chan.force_shutdown(broadcast));
				(self.get_channel_update_for_broadcast(&chan), chan.get_counterparty_node_id())
				...
				let mut chan = remove_inbound_v1_channel!(self, chan);
				self.finish_force_close_channel(chan.force_shutdown(broadcast));
				let err = Err(LightningError{err: "Channel not yet established".to_owned(), action: msgs::ErrorAction::IgnoreError});
				(err, chan.get_counterparty_node_id())
			} else if let hash_map::Entry::Occupied(chan) = peer_state.inbound_v1_channel_by_id.entry(channel_id.clone()) {
				let mut pending_events_lock = self.pending_events.lock().unwrap();
				pending_events_lock.push(events::Event::ChannelClosed {
					channel_id: chan.get().channel_id(),
				...
				let mut chan = remove_outbound_v1_channel!(self, chan);
				self.finish_force_close_channel(chan.force_shutdown(broadcast));
				let err = Err(LightningError{err: "Channel not yet established".to_owned(), action: msgs::ErrorAction::IgnoreError});
				(err, chan.get_counterparty_node_id())
			} else {
			...
			if let &PendingHTLCRouting::Forward { ref short_channel_id, .. } = routing {
				if let Some((err, mut code, chan_update)) = loop {
					...
					let chan_update_opt = if let Some((counterparty_node_id, forwarding_id)) = forwarding_chan_info_opt {
						...
						let peer_state = &mut *peer_state_lock;
						let chan = match peer_state.funded_channel_by_id.get_mut(&forwarding_id) {
							None => {
								// Channel was removed. The short_to_chan_info and funded_channel_by_id maps
	...
	fn get_channel_update_for_broadcast(&self, chan: &FundedChannel<<SP::Target as SignerProvider>::Signer>) -> Result<msgs::ChannelUpdate, LightningError> {
	...
	/// [`channel_update`]: msgs::ChannelUpdate
	/// [`internal_closing_signed`]: Self::internal_closing_signed
	fn get_channel_update_for_unicast(&self, chan: &FundedChannel<<SP::Target as SignerProvider>::Signer>) -> Result<msgs::ChannelUpdate, LightningError> {
	...
	fn get_channel_update_for_onion(&self, short_channel_id: u64, chan: &FundedChannel<<SP::Target as SignerProvider>::Signer>) -> Result<msgs::ChannelUpdate, LightningError> {
	...
	fn send_payment_along_path(&self, path: &Vec<RouteHop>, payment_hash: &PaymentHash, payment_secret: &Option<PaymentSecret>, total_value: u64, cur_height: u32, payment_id: PaymentId, keysend_preimage: &Option<PaymentPreimage>, session_priv_bytes: [u8; 32]) -> Result<(), APIError> {
		...
		let err: Result<(), _> = loop {
			...
			let mut peer_state_lock = peer_state_mutex.lock().unwrap();
			let peer_state = &mut *peer_state_lock;
			if let hash_map::Entry::Occupied(mut chan) = peer_state.funded_channel_by_id.entry(id) {
				if !chan.get().is_live() {
	...
	fn funding_transaction_generated_intern<FundingOutput: Fn(&OutboundV1Channel<<SP::Target as SignerProvider>::Signer>, &Transaction) -> Result<OutPoint, APIError>>(
		...
		let (chan, msg) = {
			let res = {
				match peer_state.outbound_v1_channel_by_id.remove(temporary_channel_id) {
					Some(chan) => {
						let funding_txo = find_funding_output(&chan, &funding_transaction)?;

						chan.get_outbound_funding_created(funding_transaction, funding_txo, &self.logger)
							.map_err(|e| if let (mut chan, ChannelError::Close(msg)) = e {
								let err = MsgHandleErrInternal::from_finish_shutdown(msg, chan.channel_id(), chan.get_user_id(), chan.force_shutdown(true), None);
								(chan, err)
							} else { unreachable!(); })
					},
			...
			match res {
				Ok(res) => res,
				Err((chan, err)) => match handle_error!(self, Err::<FundingCreated, _>(err), chan.get_counterparty_node_id()) {
					Ok(_) => { unreachable!() },
		...
		match peer_state.funded_channel_by_id.entry(chan.channel_id()) {
			hash_map::Entry::Occupied(_) => {
	...
	pub fn update_channel_config(
		...
		for channel_id in channel_ids {
			if !peer_state.funded_channel_by_id.contains_key(channel_id) {
		...
		for channel_id in channel_ids {
			let channel = peer_state.funded_channel_by_id.get_mut(channel_id).unwrap();
	...
	pub fn forward_intercepted_htlc(&self, intercept_id: InterceptId, next_hop_channel_id: &[u8; 32], next_node_id: PublicKey, amt_to_forward_msat: u64) -> Result<(), APIError> {
		...
		let next_hop_scid = {
			...
			let mut peer_state_lock = peer_state_mutex.lock().unwrap();
			let peer_state = &mut *peer_state_lock;
			match peer_state.funded_channel_by_id.get(next_hop_channel_id) {
				Some(chan) => {
					if !chan.is_usable() {
						return Err(APIError::ChannelUnavailable {
		...
		let mut phantom_receives: Vec<(u64, OutPoint, u128, Vec<(PendingHTLCInfo, u64)>)> = Vec::new();
		{
			...
			for (short_chan_id, mut pending_forwards) in forward_htlcs {
				if short_chan_id != 0 {
					...
					let mut peer_state_lock = peer_state_mutex_opt.unwrap().lock().unwrap();
					let peer_state = &mut *peer_state_lock;
					match peer_state.funded_channel_by_id.entry(forward_chan_id) {
	...
	fn update_channel_fee(&self, chan_id: &[u8; 32], chan: &mut FundedChannel<<SP::Target as SignerProvider>::Signer>, new_feerate: u32) -> NotifyOption {
	...
	pub fn maybe_update_chan_fees(&self) {
		PersistenceNotifierGuard::optionally_notify(&self.total_consistency_lock, &self.persistence_notifier, || {
			...
			for (_cp_id, peer_state_mutex) in per_peer_state.iter() {
				let mut peer_state_lock = peer_state_mutex.lock().unwrap();
				let peer_state = &mut *peer_state_lock;
				for (chan_id, chan) in peer_state.funded_channel_by_id.iter_mut() {
		...
		PersistenceNotifierGuard::optionally_notify(&self.total_consistency_lock, &self.persistence_notifier, || {
			...
			let mut pending_peers_awaiting_removal = Vec::new();
			{
				...
				for (counterparty_node_id, peer_state_mutex) in per_peer_state.iter() {
					...
					peer_state.funded_channel_by_id.retain(|chan_id, chan| {
						let chan_needs_persist = self.update_channel_fee(chan_id, chan, new_feerate);
						if chan_needs_persist == NotifyOption::DoPersist { should_persist = NotifyOption::DoPersist; }

						if let Err(e) = chan.timer_check_closing_negotiation_progress() {
							let (needs_close, err) = self.convert_funded_chan_err(e, chan, chan_id);
	...
	/// forwarding
	fn get_htlc_inbound_temp_fail_err_and_data(&self, desired_err_code: u16, chan: &FundedChannel<<SP::Target as SignerProvider>::Signer>) -> (u16, Vec<u8>) {
	...
	/// Gets an HTLC onion failure code and error data for an `UPDATE` error, given the error code
	/// that we want to return and a channel.
	fn get_htlc_temp_fail_err_and_data(&self, desired_err_code: u16, scid: u64, chan: &FundedChannel<<SP::Target as SignerProvider>::Signer>) -> (u16, Vec<u8>) {
	...
	fn fail_holding_cell_htlcs(
		...
		let (failure_code, onion_failure_data) = {
			...
			if let Some(peer_state_mutex) = per_peer_state.get(counterparty_node_id) {
				let mut peer_state_lock = peer_state_mutex.lock().unwrap();
				let peer_state = &mut *peer_state_lock;
				match peer_state.funded_channel_by_id.entry(channel_id) {
	...
	pub fn claim_funds(&self, payment_preimage: PaymentPreimage) {
		...
		for htlc in sources.iter() {
			...
			if peer_state.funded_channel_by_id.get(&chan_id).is_none() {
				valid_mpp = false;
				break;
			}
	...
	-> Result<(), (PublicKey, MsgHandleErrInternal)> {
		...
		if peer_state_opt.is_some() {
			...
			let peer_state = &mut *peer_state_lock;
			if let hash_map::Entry::Occupied(mut chan) = peer_state.funded_channel_by_id.entry(chan_id) {
	...
	fn handle_channel_resumption(&self, pending_msg_events: &mut Vec<MessageSendEvent>,
		channel: &mut FundedChannel<<SP::Target as SignerProvider>::Signer>, raa: Option<msgs::RevokeAndACK>,
	...
	fn channel_monitor_updated(&self, funding_txo: &OutPoint, highest_applied_update_id: u64, counterparty_node_id: Option<&PublicKey>) {
		...
		let peer_state = &mut *peer_state_lock;
		let mut channel = {
			match peer_state.funded_channel_by_id.entry(funding_txo.to_channel_id()){
	...
	fn unfunded_channel_count(
		...
		for (_, chan) in peer.funded_channel_by_id.iter() {
	...
	fn internal_open_channel(&self, counterparty_node_id: &PublicKey, msg: &msgs::OpenChannel) -> Result<(), MsgHandleErrInternal> {
		...
		match peer_state.inbound_v1_channel_by_id.entry(channel.channel_id()) {
			hash_map::Entry::Occupied(_) => {
				self.outbound_scid_aliases.lock().unwrap().remove(&outbound_scid_alias);
				return Err(MsgHandleErrInternal::send_err_msg_no_close("temporary_channel_id collision for the same peer!".to_owned(), msg.temporary_channel_id.clone()))
			},
			hash_map::Entry::Vacant(entry) => {
				if peer_state.outbound_v1_channel_by_id.contains_key(entry.key()) || peer_state.funded_channel_by_id.contains_key(entry.key()) {
					self.outbound_scid_aliases.lock().unwrap().remove(&outbound_scid_alias);
					return Err(MsgHandleErrInternal::send_err_msg_no_close("temporary_channel_id collision for the same peer!".to_owned(), msg.temporary_channel_id.clone()))
				}
	...
	fn internal_accept_channel(&self, counterparty_node_id: &PublicKey, msg: &msgs::AcceptChannel) -> Result<(), MsgHandleErrInternal> {
		let (value, output_script, user_id) = {
			...
			match peer_state.outbound_v1_channel_by_id.entry(msg.temporary_channel_id) {
				hash_map::Entry::Occupied(mut chan) => {
					try_outbound_v1_chan_entry!(self, chan.get_mut().accept_channel(&msg, &self.default_configuration.channel_handshake_limits, &peer_state.latest_features), chan);
	...
	fn internal_funding_created(&self, counterparty_node_id: &PublicKey, msg: &msgs::FundingCreated) -> Result<(), MsgHandleErrInternal> {
		...
		let (chan, funding_msg, monitor) =
			match peer_state.inbound_v1_channel_by_id.entry(msg.temporary_channel_id) {
				hash_map::Entry::Occupied(chan) => {
					let inbound_chan = chan.remove();
					match inbound_chan.funding_created(msg, best_block, &self.signer_provider, &self.logger) {
						Ok(res) => res,
						Err((mut inbound_chan, err)) => {
							let (drop, err) = self.convert_inbound_v1_chan_err(err, &mut inbound_chan, &msg.temporary_channel_id);
							if !drop {
								// This shouldn't really ever happen, as we always get `ChannelError::Close`, but covering this in case.
		...
		match peer_state.funded_channel_by_id.entry(funding_msg.channel_id) {
			hash_map::Entry::Occupied(_) => {
			...
			hash_map::Entry::Vacant(e) => {
				...
				let mut res = handle_new_monitor_update!(self, monitor_res, 0, peer_state_lock, peer_state,
					per_peer_state, chan, MANUALLY_REMOVING, { peer_state.funded_channel_by_id.remove(&new_channel_id) });
	...
	fn internal_funding_signed(&self, counterparty_node_id: &PublicKey, msg: &msgs::FundingSigned) -> Result<(), MsgHandleErrInternal> {
		...
		let mut peer_state_lock = peer_state_mutex.lock().unwrap();
		let peer_state = &mut *peer_state_lock;
		match peer_state.funded_channel_by_id.entry(msg.channel_id) {
			hash_map::Entry::Occupied(mut chan) => {
	...
	fn internal_shutdown(&self, counterparty_node_id: &PublicKey, msg: &msgs::Shutdown) -> Result<(), MsgHandleErrInternal> {
		...
		let result: Result<(), _> = loop {
			...
			let mut peer_state_lock = peer_state_mutex.lock().unwrap();
			let peer_state = &mut *peer_state_lock;
			match peer_state.funded_channel_by_id.entry(msg.channel_id.clone()) {
				hash_map::Entry::Occupied(mut chan_entry) => {
	...
	fn internal_update_add_htlc(&self, counterparty_node_id: &PublicKey, msg: &msgs::UpdateAddHTLC) -> Result<(), MsgHandleErrInternal> {
		...
		match peer_state.funded_channel_by_id.entry(msg.channel_id) {
			hash_map::Entry::Occupied(mut chan) => {
				...
				let create_pending_htlc_status = |chan: &FundedChannel<<SP::Target as SignerProvider>::Signer>, pending_forward_info: PendingHTLCStatus, error_code: u16| {
	...
	fn internal_channel_update(&self, counterparty_node_id: &PublicKey, msg: &msgs::ChannelUpdate) -> Result<NotifyOption, MsgHandleErrInternal> {
		...
		let mut peer_state_lock = peer_state_mutex_opt.unwrap().lock().unwrap();
		let peer_state = &mut *peer_state_lock;
		match peer_state.funded_channel_by_id.entry(chan_id) {
			hash_map::Entry::Occupied(mut chan) => {
					...
					MonitorEvent::UpdateFailed(funding_outpoint) => {
						...
						if let Some(counterparty_node_id) = counterparty_node_id_opt {
							...
							if let Some(peer_state_mutex) = per_peer_state.get(&counterparty_node_id) {
								...
								let peer_state = &mut *peer_state_lock;
								let pending_msg_events = &mut peer_state.pending_msg_events;
								if let hash_map::Entry::Occupied(chan_entry) = peer_state.funded_channel_by_id.entry(funding_outpoint.to_channel_id()) {
		...
		'peer_loop: loop {
			...
			for (_cp_id, peer_state_mutex) in per_peer_state.iter() {
				'chan_loop: loop {
					...
					for (channel_id, chan) in peer_state.funded_channel_by_id.iter_mut() {
						let counterparty_node_id = chan.get_counterparty_node_id();
						...
						if let Some(monitor_update) = monitor_opt {
							...
							let res = handle_new_monitor_update!(self, update_res, update_id,
								...
								peer_state.funded_channel_by_id.remove(&channel_id));
							if res.is_err() {
	...
	fn maybe_generate_initial_closing_signed(&self) -> bool {
		...
		let mut has_update = false;
		{
			...
			for (_cp_id, peer_state_mutex) in per_peer_state.iter() {
				...
				let peer_state = &mut *peer_state_lock;
				let pending_msg_events = &mut peer_state.pending_msg_events;
				peer_state.funded_channel_by_id.retain(|channel_id, chan| {
					match chan.maybe_propose_closing_signed(&self.fee_estimator, &self.logger) {
						...
						Err(e) => {
							has_update = true;
							let (close_channel, res) = self.convert_funded_chan_err(e, chan, channel_id);
	...
	pub fn compute_inflight_htlcs(&self) -> InFlightHtlcs {
		...
		for (_cp_id, peer_state_mutex) in per_peer_state.iter() {
			let mut peer_state_lock = peer_state_mutex.lock().unwrap();
			let peer_state = &mut *peer_state_lock;
			for chan in peer_state.funded_channel_by_id.values() {
	...
	fn convert_funded_chan_err(
		&self, err: ChannelError, channel: &mut FundedChannel<<<SP as Deref>::Target as SignerProvider>::Signer>,
		channel_id: &[u8; 32]) -> (bool, MsgHandleErrInternal) {
		match err {
			...
			ChannelError::Close(msg) => {
				...
				self.update_maps_on_funded_chan_removal(channel);
	...
	/// Returns (boolean indicating if we should remove the `InboundV1Channel` object from memory, a mapped error)
	fn convert_inbound_v1_chan_err(
		&self, err: ChannelError, channel: &mut InboundV1Channel<<<SP as Deref>::Target as SignerProvider>::Signer>,
		channel_id: &[u8; 32]) -> (bool, MsgHandleErrInternal) {
		match err {
			ChannelError::Warn(msg) => {
				(false, MsgHandleErrInternal::from_chan_no_close(ChannelError::Warn(msg), channel_id.clone()))
			...
			ChannelError::Close(msg) => {
				log_error!(self.logger, "Closing channel {} due to close-required error: {}", log_bytes!(channel_id[..]), msg);
				self.update_maps_on_pending_chan_removal(channel_id, channel.outbound_scid_alias());
				let shutdown_res = channel.force_shutdown(true);
				(true, MsgHandleErrInternal::from_finish_shutdown(msg, *channel_id, channel.get_user_id(),
					shutdown_res, None))
			},
	...
	/// Returns (boolean indicating if we should remove the `InboundV1Channel` object from memory, a mapped error)
	fn convert_outbound_v1_chan_err(
		&self, err: ChannelError, channel: &mut OutboundV1Channel<<<SP as Deref>::Target as SignerProvider>::Signer>,
		channel_id: &[u8; 32]) -> (bool, MsgHandleErrInternal) {
		match err {
			ChannelError::Warn(msg) => {
				(false, MsgHandleErrInternal::from_chan_no_close(ChannelError::Warn(msg), channel_id.clone()))
	...
	fn remove_outbound_scid_alias(&self, outbound_scid_alias: u64) {
		let alias_removed = self.outbound_scid_aliases.lock().unwrap().remove(&outbound_scid_alias);
		debug_assert!(alias_removed);
	}

	fn update_maps_on_pending_chan_removal(&self, channel_id: &[u8; 32], outbound_scid_alias: u64) {
		self.id_to_peer.lock().unwrap().remove(channel_id);
		self.remove_outbound_scid_alias(outbound_scid_alias);
		self.short_to_chan_info.write().unwrap().remove(&outbound_scid_alias);
	}

	fn update_maps_on_funded_chan_removal(&self, channel: &mut FundedChannel<<<SP as Deref>::Target as SignerProvider>::Signer>) {
		self.id_to_peer.lock().unwrap().remove(&channel.channel_id());
		let mut short_to_chan_info = self.short_to_chan_info.write().unwrap();
		if let Some(short_id) = channel.get_short_channel_id() {
			short_to_chan_info.remove(&short_id);
			...
			// opening a million channels with us which are closed before we ever reach the funding
			// stage.
			self.remove_outbound_scid_alias(channel.outbound_scid_alias());
		}
		short_to_chan_info.remove(&channel.outbound_scid_alias());
	}
}
	...
	L::Target: Logger,
{
	...
	/// the function.
	fn do_chain_event<FN: Fn(&mut FundedChannel<<SP::Target as SignerProvider>::Signer>) -> Result<(Option<msgs::ChannelReady>, Vec<(HTLCSource, PaymentHash)>, Option<msgs::AnnouncementSignatures>), ClosureReason>>
		...
		let mut timed_out_htlcs = Vec::new();
		{
			...
			for (_cp_id, peer_state_mutex) in per_peer_state.iter() {
				...
				let peer_state = &mut *peer_state_lock;
				let pending_msg_events = &mut peer_state.pending_msg_events;
				peer_state.funded_channel_by_id.retain(|_, channel| {
					let res = f(channel);
	...
	fn peer_disconnected(&self, counterparty_node_id: &PublicKey) {
		...
		let remove_peer = {
			...
			if let Some(peer_state_mutex) = per_peer_state.get(counterparty_node_id) {
				...
				let peer_state = &mut *peer_state_lock;
				let pending_msg_events = &mut peer_state.pending_msg_events;
				peer_state.funded_channel_by_id.retain(|_, chan| {
				...
				peer_state.inbound_v1_channel_by_id.retain(|_, chan| {
					self.update_maps_on_pending_chan_removal(&chan.channel_id(), chan.outbound_scid_alias());
					self.pending_events.lock().unwrap().push(events::Event::ChannelClosed {
						channel_id: chan.channel_id(),
						user_channel_id: chan.get_user_id(),
			...
			match peer_state_lock.entry(counterparty_node_id.clone()) {
				hash_map::Entry::Vacant(e) => {
					...
					e.insert(Mutex::new(PeerState {
						funded_channel_by_id: HashMap::new(),
				...
				hash_map::Entry::Occupied(e) => {
					...
					if inbound_peer_limited &&
						...
						peer_state.funded_channel_by_id.len()
					{
						return Err(());
					}
	...
	fn handle_error(&self, counterparty_node_id: &PublicKey, msg: &msgs::ErrorMessage) {
		...
		if msg.channel_id == [0; 32] {
			let channel_ids: Vec<[u8; 32]> = {
				...
				let mut peer_state_lock = peer_state_mutex_opt.unwrap().lock().unwrap();
				let peer_state = &mut *peer_state_lock;
				peer_state.funded_channel_by_id.keys().cloned().collect()
			};
			for channel_id in channel_ids {
	...
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		...
		let mut serializable_peer_count: u64 = 0;
		{
			...
			for (_, peer_state_mutex) in per_peer_state.iter() {
				...
				number_of_channels += peer_state.funded_channel_by_id.len();
				for (_, channel) in peer_state.funded_channel_by_id.iter() {
	...
	L::Target: Logger,
{
	fn read<Reader: io::Read>(reader: &mut Reader, mut args: ChannelManagerReadArgs<'a, M, T, ES, NS, SP, F, R, L>) -> Result<Self, DecodeError> {
		...
		let mut peer_channels: HashMap<PublicKey, HashMap<[u8; 32], FundedChannel<<SP::Target as SignerProvider>::Signer>>> = HashMap::with_capacity(cmp::min(channel_count as usize, 128));
		let mut id_to_peer = HashMap::with_capacity(cmp::min(channel_count as usize, 128));
		let mut short_to_chan_info = HashMap::with_capacity(cmp::min(channel_count as usize, 128));
		let mut channel_closures = Vec::new();
		for _ in 0..channel_count {
			let mut channel: FundedChannel<<SP::Target as SignerProvider>::Signer> = FundedChannel::read(reader, (
		...
		for _ in 0..peer_count {
			...
			let peer_state = PeerState {
				funded_channel_by_id: peer_channels.remove(&peer_pubkey).unwrap_or(HashMap::new()),
				outbound_v1_channel_by_id: HashMap::new(),
				inbound_v1_channel_by_id: HashMap::new(),
				...
				if let Some((payment_purpose, claimable_htlcs)) = claimable_htlcs.remove(&payment_hash) {
					...
					for claimable_htlc in claimable_htlcs {
						...
						if let Some(peer_node_id) = id_to_peer.get(&previous_channel_id){
							...
							let mut peer_state_lock = peer_state_mutex.lock().unwrap();
							let peer_state = &mut *peer_state_lock;
							if let Some(channel) = peer_state.funded_channel_by_id.get_mut(&previous_channel_id) {
