...
macro_rules! convert_prefunded_chan_err {
	($self: ident, $err: expr, $channel_context: expr, $channel_id: expr) => {
		match $err {
...
macro_rules! try_v1_outbound_chan_entry {
	($self: ident, $res: expr, $entry: expr) => {
		match $res {
			Ok(res) => res,
			Err(e) => {
				let (drop, res) = convert_prefunded_chan_err!($self, e, $entry.get_mut().context, $entry.key());
				if drop {
					$entry.remove_entry();
				}
				return Err(res);
	...
	L::Target: Logger,
{
	...
	fn funding_transaction_generated_intern<FundingOutput: Fn(&OutboundV1Channel<<SP::Target as SignerProvider>::Signer>, &Transaction) -> Result<OutPoint, APIError>>(
		...
		let mut peer_state_lock = peer_state_mutex.lock().unwrap();
		let peer_state = &mut *peer_state_lock;
		let (msg, chan) = match peer_state.outbound_v1_channel_by_id.remove(temporary_channel_id) {
			Some(mut chan) => {
	...
	fn internal_funding_signed(&self, counterparty_node_id: &PublicKey, msg: &msgs::FundingSigned) -> Result<(), MsgHandleErrInternal> {
		...
		let mut peer_state_lock = peer_state_mutex.lock().unwrap();
		let peer_state = &mut *peer_state_lock;
		match peer_state.outbound_v1_channel_by_id.remove(&msg.channel_id) {
			Some(chan) => {
				let funding_signed_res = chan.funding_signed(&msg, best_block, &self.signer_provider, &self.logger)
					.map_err(|(mut chan, err)| convert_prefunded_chan_err!(self, err, &mut chan.context, &chan.context.channel_id()));
				match funding_signed_res {
					Ok((chan, monitor)) => {
						let monitor_res = self.chain_monitor.watch_channel(chan.context.get_funding_txo().unwrap(), monitor);
						match peer_state.channel_by_id.entry(chan.context.channel_id()) {
							hash_map::Entry::Occupied(_) => {
								panic!("Got duplicate funding txid from funding_transaction_generated_intern");
							},
							hash_map::Entry::Vacant(entry) => {
								let mut chan = entry.insert(chan);
								let mut res = handle_new_monitor_update!(self, monitor_res, 0, peer_state_lock, peer_state,
									per_peer_state, &mut chan, MANUALLY_REMOVING, { peer_state.channel_by_id.remove(&msg.channel_id) });
			...
			None => return Err(MsgHandleErrInternal::send_err_msg_no_close("Failed to find corresponding channel".to_owned(), msg.channel_id))
		}
	}
	...
	// 		Some(chan) => {
				// let funding_signed_res = chan.funding_signed(&msg, best_block, &self.signer_provider, &self.logger)
				// 	.map_err(|(chan, err)| convert_prefunded_chan_err!(self, err, &chan.context, &chan.context.channel_id()));
	...
	// 								panic!("id_to_peer map already contained funding txid, which shouldn't be possible");
	// 							}
	// 							let chan = entry.insert(chan);
	// 							let mut res = handle_new_monitor_update!(self, monitor_res, 0, peer_state_lock, peer_state,
	// 								per_peer_state, chan, MANUALLY_REMOVING, { peer_state.channel_by_id.remove(&chan.context.channel_id()) });
	// 							// We do not propagate the monitor update to the user as it would be for a monitor
	// 							// that we didn't manage to store.
	// 							if let Err(MsgHandleErrInternal { shutdown_finish: Some((res, _)), .. }) = &mut res {
	// 								res.0.take();
	// 							}
	// 							res
	...
	// 			}
	// 		},
	// 		None => Err(MsgHandleErrInternal::send_err_msg_no_close("Failed to find corresponding channel".to_owned(), msg.channel_id)),
	// 	}
	// }
