		...
		match peer_state.outbound_v1_channel_by_id.remove(&msg.channel_id) {
			Some(chan) => {
				let funding_signed_res = chan.funding_signed(&msg, best_block, &self.signer_provider, &self.logger)
					.map_err(|(mut chan, err)| convert_chan_err!(self, err, &mut chan.context, &chan.context.channel_id(), PREFUNDED));
					...
					Ok((chan, monitor)) => {
						...
						match peer_state.channel_by_id.entry(chan.context.channel_id()) {
							...
							hash_map::Entry::Vacant(entry) => {
								...
								let channel_id = chan.context.channel_id();
								let mut res = handle_new_monitor_update!(self, monitor_res, 0, peer_state_lock, peer_state,
									per_peer_state, &mut chan, MANUALLY_REMOVING, { peer_state.channel_by_id.remove(&channel_id) });
