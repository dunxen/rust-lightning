...
mod tests {
	...
	#[cfg(any(dual_funding, splicing))]
	use crate::events::OpenChannelRequestAcceptorFunds;
	...
	fn do_test_v2_channel_establishment(session: V2ChannelEstablishmentTestSession) {
		let acceptor_will_fund = session.acceptor_funding_satoshis > 0;
		...
		nodes[0].node.create_dual_funded_channel(
			nodes[1].node.get_our_node_id(), session.initiator_funding_satoshis, initiator_funding_inputs,
		...
		if acceptor_will_fund {
			...
			let temporary_channel_id = if let Event::OpenChannelRequest {
				...
				assert!(matches!(acceptor_funds, OpenChannelRequestAcceptorFunds::DualFunded));
				temporary_channel_id
			} else { panic!(); };
			...
			nodes[1].node.accept_inbound_channel_with_contribution(
				&temporary_channel_id, &nodes[0].node.get_our_node_id(), 1337, session.acceptor_funding_satoshis, acceptor_funding_inputs
			).unwrap();
		}
		...
		assert_eq!(input_value, session.initiator_input_value_satoshis);
		...
		if acceptor_will_fund {
			...
			let input_value = tx_add_input_msg.prevtx.as_transaction().output[tx_add_input_msg.prevtx_out as usize].value;
			assert_eq!(input_value, session.acceptor_input_value_satoshis);
		...
		if acceptor_will_fund {
			let tx_add_output_msg = get_event_msg!(&nodes[1], MessageSendEvent::SendTxAddOutput, nodes[0].node.get_our_node_id());
			nodes[0].node.handle_tx_add_output(&nodes[1].node.get_our_node_id(), &tx_add_output_msg);
		} else {
			let tx_complete_msg = get_event_msg!(nodes[1], MessageSendEvent::SendTxComplete, nodes[0].node.get_our_node_id());
			nodes[0].node.handle_tx_complete(&nodes[1].node.get_our_node_id(), &tx_complete_msg);
		}
		...
		let tx_signatures_exchange = |first: usize, second: usize| {
			let msg_events = nodes[first].node.get_and_clear_pending_msg_events();
			...
			let events_0 = nodes[first].node.get_and_clear_pending_events();
			...
			let events_1 = nodes[second].node.get_and_clear_pending_events();
		...
		if session.initiator_funding_satoshis > session.acceptor_funding_satoshis
			|| session.initiator_funding_satoshis == session.acceptor_funding_satoshis
	...
	fn test_v2_channel_establishment() {
		...
		do_test_v2_channel_establishment(V2ChannelEstablishmentTestSession {
			initiator_funding_satoshis: 50_000,
